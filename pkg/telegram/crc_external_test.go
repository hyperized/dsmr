...
func TestComputeCRC(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := telegram.ComputeCRC(tt.input)
...
func TestValidCRC(t *testing.T) {
	data := []byte("123456789")
	hex := fmt.Sprintf("%04X", telegram.ComputeCRC(data))

	assert.True(t, telegram.ValidCRC(data, hex))
	assert.True(t, telegram.ValidCRC(data, "bb3d")) // case-insensitive
	assert.False(t, telegram.ValidCRC(data, "0000"))
	assert.False(t, telegram.ValidCRC(data, "BB3"))   // too short
	assert.False(t, telegram.ValidCRC(data, "BB3DE")) // too long
	assert.False(t, telegram.ValidCRC(data, "ZZZZ"))  // not hex
}
...
func TestExampleTelegramTwoCRC(t *testing.T) {
	...
	assert.True(t, telegram.ValidCRC(buf.Bytes(), "E47C"),
		"CRC mismatch: computed %04X", telegram.ComputeCRC(buf.Bytes()))
}
...
func TestCRCRoundTrip(t *testing.T) {
	...
	for _, data := range inputs {
		crcVal := telegram.ComputeCRC(data)
		assert.True(t, telegram.ValidCRC(data, fmt.Sprintf("%04X", crcVal)))
	}
}
